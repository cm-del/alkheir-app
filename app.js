// ──────────────── DATABASE ────────────────
let DB = JSON.parse(localStorage.getItem('akh_v10') || JSON.stringify({
...
function save(){try{localStorage.setItem('akh_v10',JSON.stringify(DB));}catch(e){toast('تخزين ممتلئ!','err');}}
...
function fridays(d){
    // Correct calculation: count Fridays from startDate up to today inclusive
    let start = new Date(d + 'T00:00:00');
...
function delWithConfirm(type){
    ...
    confirmCallback=()=>{
        ...
        if(type==='batch')deleteBatchConfirm();
        if(type==='photo')delPhotoConfirmed();
    };
    document.getElementById('confirm-yes').onclick=()=>{confirmCallback();closeModal('confirm');};
    openModal('confirm');
}
let _pendingDelBatchId = null;
let _pendingDelPhotoId = null;
function confirmDeleteBatch(id){_pendingDelBatchId=id; delWithConfirm('batch');}
function deleteBatchConfirm(){
    const id=_pendingDelBatchId; if(!id)return;
    DB.batches=DB.batches.filter(b=>b.id!==id);
    // remove related records
    ['weights','feed','deaths','sales'].forEach(k=>DB[k]=DB[k].filter(r=>r.batchId!==id));
    // remove photos from IndexedDB & DB
    const relatedPhotos = DB.photos.filter(p=>p.batchId===id);
    relatedPhotos.forEach(async p=>{ await idbKeyval.del('ph_'+p.id); });
    DB.photos = DB.photos.filter(p=>p.batchId!==id);
    save();
    _pendingDelBatchId=null;
    renderFarmTree();
    renderDash();
    toast('🗑 تم حذف الدفعة');
}
function confirmDeletePhoto(id){_pendingDelPhotoId=id; delWithConfirm('photo');}
function delPhotoConfirmed(){
    const id=_pendingDelPhotoId; if(!id)return;
    idbKeyval.del('ph_'+id);
...
function resetAllData(){
    ...
    localStorage.removeItem('akh_v10');
...
function deleteFarm(){
    ...
    // Get all hangars of this farm
    const hi=DB.hangars.filter(h=>h.farmId===id).map(h=>h.id);
    // Get all batches in those hangars
    const bi=DB.batches.filter(b=>hi.includes(b.hangarId)).map(b=>b.id);
    // Remove photos from IndexedDB
    const relatedPhotos = DB.photos.filter(p=>bi.includes(p.batchId));
    relatedPhotos.forEach(async p=>{ await idbKeyval.del('ph_'+p.id); });
    // Remove records
    DB.farms=DB.farms.filter(x=>x.id!==id);
    DB.hangars=DB.hangars.filter(h=>h.farmId!==id);
    DB.batches=DB.batches.filter(b=>!hi.includes(b.hangarId));
    ['weights','feed','deaths','sales'].forEach(k=>DB[k]=DB[k].filter(r=>!bi.includes(r.batchId)));
    DB.photos=DB.photos.filter(p=>!bi.includes(p.batchId));
    DB.feedStore=DB.feedStore.filter(r=>r.farmId!==id);
    DB.staff=DB.staff.filter(s=>s.farmId!==id);
    DB.tempLogs=DB.tempLogs.filter(t=>!hi.includes(t.hangarId));
    if(cFarm===id)cFarm=DB.farms.length?DB.farms[0].id:null;
    save();
    closeModal('editFarm');
    renderFarmBar();renderFarmTree();renderDash();
    toast('🗑 تم الحذف');
}
...
function openEditHangar(id){const h=DB.hangars.find(x=>x.id===id);if(!h)return;document.getElementById('eh-id').value=h.id;document.getElementById('eh-name').value=h.name;document.getElementById('eh-cap').value=h.cap===0 ? 0 : (h.cap || '');document.getElementById('eh-area').value=h.area===0 ? 0 : (h.area || '');document.getElementById('eh-win').value=h.windows===0 ? 0 : (h.windows || '');document.getElementById('eh-fans').value=h.fans===0 ? 0 : (h.fans || '');document.getElementById('eh-fcap').value=h.fanCap===0 ? 0 : (h.fanCap || '');openModal('editHangar');}
...
function saveFeed(){
    if(!validateNumber('#fd-bags','الشيكاير',0.5))return;
    const b=document.getElementById('fd-b').value,bags=+document.getElementById('fd-bags').value,d=document.getElementById('fd-d').value||todayS();
    if(!b){toast('اختر دفعة','err');return;}
    const bt=DB.batches.find(x=>x.id===b),hn=bt?DB.hangars.find(h=>h.id===bt.hangarId):null,fid=hn?hn.farmId:null;
    const kg=bags*KPB;
    if(fid){
        const st=getFStock(fid);
        if(kg > st.kg){
            toast('❌ المخزون غير كافٍ!', 'err');
            return;
        }
    }
    DB.feed.push({id:uid(),batchId:b,bags,qty:kg,type:document.getElementById('fd-type').value,cost:0,date:d});
    if(fid)DB.feedStore.push({id:uid(),farmId:fid,date:d,bags,kg,txType:'out',batchId:b,feedType:document.getElementById('fd-type').value,note:'صرف'});
    save();closeModal('addFeed');renderDTab();renderDash();renderStore();
    toast('🌾 تم الصرف');
    // Low stock warning after saving
    if(fid){
        const newStock = getFStock(fid);
        if(newStock.bags < 10) sendNotification('⚠️ تنبيه مخزون', 'مخزن العلف منخفض: '+newStock.bags.toFixed(1)+' شيكارة');
    }
}
...
function saveDeath(){if(!validateNumber('#dt-cnt','العدد',1))return;const b=document.getElementById('dt-b').value,c=+document.getElementById('dt-cnt').value,d=document.getElementById('dt-d').value||todayS(),p=document.getElementById('dt-per').value;if(!b){toast('اختر دفعة','err');return;}const ex=DB.deaths.find(x=>x.batchId===b&&x.date===d&&x.period===p);if(ex){ex.count+=c;}else{DB.deaths.push({id:uid(),batchId:b,count:c,date:d,period:p,reason:document.getElementById('dt-rsn').value});}const bt=DB.batches.find(x=>x.id===b);if(bt)bt.count=Math.max(0,bt.count-c);save();closeModal('addDeath');renderDTab();renderDash();toast('💀 تم التسجيل');
    ...
    const weeklyDeaths = DB.deaths.filter(d=>d.batchId===b && new Date(d.date+'T12:00:00')>=weekAgo).reduce((s,x)=>s+x.count,0);
    if(bt && bt.startCount && (weeklyDeaths/bt.startCount)*100 >= 3) {
        sendNotification('⚠️ نفوق مرتفع', bt.name + ': نسبة النفوق تجاوزت 3% هذا الأسبوع');
    }
}
...
function renderDTab(){
    ...
    if(t==='w'){
        ...
        h+='<div class="card"><div class="tw"><table><thead><tr><th>دفعة</th><th>وزن</th><th>تاريخ</th><th></th></tr></thead><tbody>';
...
function renderTTab(){
    ...
    if(cTTab==='log'){
        ...
        let h='';hs.forEach(hn=>{if(!hn)return;const bts=DB.batches.filter(b=>b.hangarId===hn.id&&b.active),birds=bts.reduce((s,b)=>s+b.count,0);
            // ventilation requirement: ~4 m³/h per kg live weight (simplified)
            const lastWeights = bts.map(b=>{const ws=DB.weights.filter(w=>w.batchId===b.id).sort((a,c)=>c.date>a.date?1:-1);return ws.length?ws[0].weight:null;}).filter(Boolean);
            const avgWeight = lastWeights.length ? lastWeights.reduce((a,b)=>a+b,0)/lastWeights.length : 0.5; // fallback 0.5 kg
            const estimatedLiveWeight = birds * avgWeight;
            const requiredCapacity = estimatedLiveWeight * 4; // m³/h
            const actual = (hn.fans||0) * (hn.fanCap||0);
            const ok = actual >= requiredCapacity;
            h+='<div class="card"><div class="card-title">💨 '+hn.name+'</div><div>طيور: '+fmt(birds)+' | الوزن الحي المقدر: '+fmt(Math.round(estimatedLiveWeight))+' كجم</div><div>السعة المطلوبة: '+fmt(Math.round(requiredCapacity))+' م³/س | السعة الحالية: '+fmt(actual)+' م³/س</div><div class="'+(ok?'ao':'alert')+' alert">'+(ok?'✅ تهوية كافية':'⚠️ راجع التهوية')+'</div></div>';
        });
        el.innerHTML=h||'<div class="empty">لا عنابر</div>';
    }
}
...
function renderAnalytics(t,bts){
    ...
    if(t==='fcr'){
        ...
        setTimeout(()=>{const ctx=document.getElementById('growC');if(!ctx)return;const colors=['#00e272','#ffb703','#38b6ff'];const ds=[];bts.forEach((b,i)=>{const ws=DB.weights.filter(w=>w.batchId===b.id).sort((a,c)=>a.date>c.date?1:-1);if(!ws.length)return;ds.push({label:b.name,data:ws.map(w=>({x:dAge(b.date)-dAge(w.date),y:w.weight})),borderColor:colors[i%3],tension:0.4,pointRadius:3});});
        if(ds.length)new Chart(ctx,{type:'line',data:{datasets:ds},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#dff5e8'}}},scales:{x:{ticks:{color:'#4a7a5e'},grid:{color:'#1c3424'}},y:{ticks:{color:'#4a7a5e'},grid:{color:'#1c3424'}}}}});},200);
    }
}
...
function renderMore(){
    ...
    if(cMoreTab==='photos'){h+='<button class="btn btn-g" onclick="openModal(\'addPhoto\');fillBS(\'ph-b\',true);">+ صورة</button>';h+='<div class="photo-grid">';
        DB.photos.slice().reverse().forEach(p=>{const src=p.src||'';h+='<div class="photo-item"><img src="'+src+'" onclick="window.open(\''+src+'\')"><button class="photo-del" onclick="confirmDeletePhoto(\''+p.id+'\')">×</button></div>';});
...
function renderDash(){
    ...
    let al='';
    if(cFarm){const st=getFStock(cFarm);if(st.bags<10){al+='<div class="alert">⚠️ مخزن منخفض: '+st.bags.toFixed(1)+' شيكارة</div>';sendNotification('⚠️ مخزن منخفض', st.bags.toFixed(1)+' شيكارة');}}
...
function renderFarmTree(){
    ...
    el.innerHTML=DB.farms.map(f=>{const hs=DB.hangars.filter(hn=>hn.farmId===f.id),st=getFStock(f.id),si=getStaffInfo(f.id);return '<div class="card" style="border-right:3px solid var(--a2)"><div class="rb"><div><b style="color:var(--a2)">🏡 '+f.name+'</b><br><small>🌾'+st.bags.toFixed(1)+' ش · 👥'+si.rows.length+'</small></div><div><button class="btn btn-sm" onclick="openAddHangar(\''+f.id+'\')">+ عنبر</button><button class="ebtn" onclick="openEditFarm(\''+f.id+'\')">✏️</button></div></div>'+hs.map(hn=>{const bts=DB.batches.filter(b=>b.hangarId===hn.id);const act=bts.filter(b=>b.active);const birds=act.reduce((s,b)=>s+b.count,0);return '<div class="hi"><div class="rb"><div><b>🏠 '+hn.name+'</b><br><small>'+act.length+' دفعة · '+fmt(birds)+' طير</small></div><div><button class="btn btn-sm btn-outline" onclick="openAddBatch(\''+hn.id+'\')">+ دفعة</button><button class="ebtn" onclick="openEditHangar(\''+hn.id+'\')">✏️</button></div></div>'+act.map(b=>'<div class="bc" style="margin-top:6px"><div class="rb"><div><b>'+b.name+'</b><br><small>'+dAge(b.date)+' يوم · '+fmt(b.count)+' طير</small></div><div><span class="badge bg">نشطة</span><button class="btn btn-sm btn-r" onclick="confirmDeleteBatch(\''+b.id+'\')">🗑</button></div></div></div>').join('')+'</div>';}).join('')+'</div>';}).join('');
}
